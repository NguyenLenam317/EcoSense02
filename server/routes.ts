...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/chat/history", async (req: Request, res: Response) => {
    try {
      // Extract user ID from request query or headers, default to 0
      const userId = req.query.userId || req.headers['x-user-id'] || '0';
      ...
      // Always return chat history, even if empty
